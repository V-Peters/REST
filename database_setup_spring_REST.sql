...
DROP DATABASE IF EXISTS `springstudent`;
CREATE DATABASE `springstudent`;
...
DROP TABLE IF EXISTS `springstudent`.`meeting`;
CREATE TABLE `springstudent`.`meeting` (
...
DROP TABLE IF EXISTS `springstudent`.`user`;
CREATE TABLE `springstudent`.`user` (
  `id` int NOT NULL AUTO_INCREMENT,
...
DROP TABLE IF EXISTS `springstudent`.`role`;
CREATE TABLE `springstudent`.`role` (
  `id` int NOT NULL,
...
DROP TABLE IF EXISTS `springstudent`.`user_role`;
CREATE TABLE `springstudent`.`user_role` (
  `id` int NOT NULL AUTO_INCREMENT,
  `id_user` INT NOT NULL UNIQUE,
  `id_role` INT NOT NULL,
  PRIMARY KEY (`id`),
  FOREIGN KEY (`id_user`) REFERENCES `springstudent`.`user`(`id`) ON DELETE CASCADE,
  FOREIGN KEY (`id_role`) REFERENCES `springstudent`.`role`(`id`) ON DELETE CASCADE
);

#########################################################################################################################################

DROP TABLE IF EXISTS `springstudent`.`meeting_user`;
CREATE TABLE `springstudent`.`meeting_user` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  `last_updated` datetime DEFAULT now(),
  PRIMARY KEY (`id`),
  FOREIGN KEY (`id_meeting`) REFERENCES `springstudent`.`meeting`(`id`) ON DELETE CASCADE,
  FOREIGN KEY (`id_user`) REFERENCES `springstudent`.`user`(`id`) ON DELETE CASCADE
);
...
INSERT INTO `springstudent`.`meeting` (
    `name`,
    `date_time`) 
VALUES
...
INSERT INTO `springstudent`.`user` (
    `username`,
    `password`,
    `firstname`,
    `lastname`,
...
INSERT INTO `springstudent`.`role` (
    `id`,
    `name`)
VALUES
	(1, 'ROLE_ADMIN'),
...
INSERT INTO `springstudent`.`user_role` (
    `id_user`,
    `id_role`)
VALUES
	(1, 1),
...
INSERT INTO `springstudent`.`meeting_user` (
    `id_meeting`,
    `id_user`) 
VALUES
	(1, 2),
